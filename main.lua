...
local function moveToAndInteract(obj)
    ...
    elseif distance < moveThreshold then
        -- Usar o módulo explorer em vez de pathfinder.request_move
...
on_update(function()
    if plugin_enabled then
        if teleporting then
            ...
            -- Lógica de teleporte aqui (você pode adicionar o que for necessário)
        else
            local local_player = get_local_player()
            if local_player then
                ...
                if current_in_helltide then
                    ...
                    if explorer_active then
                        local player_pos = get_player_position()
                        local target = explorer.get_target()
                        if target then
                            local distance_to_target = player_pos:dist_to(target)
                            if distance_to_target < 2 then
                                explorer.disable()
                                explorer_active = false
                                console.print("Explorer module finished, resuming normal movement")
                                -- Tente interagir novamente
                                interactWithObjects()
                            end
                        else
                            console.print("No target set for explorer module")
                        end
                    else
