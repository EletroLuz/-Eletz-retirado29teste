    ...
    if target_position then
        ...
        if not current_path or #current_path == 0 or path_index > #current_path or 
           (current_time - last_path_calculation_time > path_recalculation_interval) then
            ...
            if raw_path then
                current_path = smooth_path(raw_path, 0.5)  -- Apply path smoothing
            else
...
on_render(function()
    ...
    if enabled then
        ...
        table.insert(render_buffer, {text = "Mode: " .. exploration_mode, position = vec2:new(10, 10), size = 20, color = color_white(255)})
    end
end)
...
return {
    ...
    get_target = get_target,  -- Adicione esta linha
